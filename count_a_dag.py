...
# Step a: Generate 100 text files with random letters
def generate_files():
...
# Function to count 'a's in a single file
...
# Step b: Count 'a's in each file using ThreadPoolExecutor
def count_a_parallel():
    with ThreadPoolExecutor() as executor:
        executor.map(count_a_in_file, range(1, 101))

# Step c: Sum the counts from all .res files
def sum_results():
    total_count = 0
    for i in range(1, 101):
